...
local function require_serializer(serializer_name)
    if serializer_name then
        local ok, module = pcall(require, "etcd.core.serializers." .. serializer_name)
        if ok then
            return module
        end
    end

    return require("etcd.core.serializers.json")
end
...
function _M.new(opts)
    ...
    if protocol == "v3" then
        -- if opts special the api_prefix, not need to check version
