...
LUA_CLIB = skynet \
  client \
  bson md5 sproto lpeg version kvstore $(TLS_MODULE)
...
$(LUA_CLIB_PATH)/version.so : lualib-src/lua-version.c| $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) $^ -o $@ -Iskynet-src -Iservice-src -Ilualib-src

$(LUA_CLIB_PATH)/kvstore.so : lualib-src/lua-kvstore.c| $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) $^ -o $@ -Iskynet-src -Iservice-src -Ilualib-src
